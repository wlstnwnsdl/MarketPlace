...
2. **프롬프트 등록** — 파일 타입별 업로드 (CLAUDE.md, agents/*.md, skills/*.md, commands/*.md), 제목·설명·태그·가격 설정, 공개 상태(대기/공개/미공개) 선택
3. **프롬프트 탐색** — 타입별 필터(CLAUDE.md / agents / skills / commands), 역할별 필터(개발자 / 기획자 / 디자이너 / PM / 마케터 / 영업), 기술 스택 태그 검색 (제목·설명·태그 전체 검색)
4. **상세 페이지** — 프롬프트 미리보기(최대 5줄 공개), 사용 예시, 판매자 정보
