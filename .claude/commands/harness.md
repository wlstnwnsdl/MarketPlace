...
단계별로 사용할 모델이 다르다. 반드시 아래 규칙을 따라라:

| 단계 | 모델 | 이유 |
|------|------|------|
| A. 탐색 — `docs/` 파일 읽기 | `claude-haiku-4-5-20251001` | 문서 읽기는 빠른 모델로 충분 |
| B. 논의 — 읽은 내용 분석·설계 결정 | `claude-sonnet-4-6` | 트레이드오프 분석과 설계에는 추론 능력이 필요 |
| D/E. Phase step 실제 개발 실행 | `claude-haiku-4-5-20251001` | execute.py가 `--model` 플래그로 자동 적용 |
...
**이 단계는 `claude-haiku-4-5-20251001` 모델로 수행한다.**

### B. 논의

구현을 위해 구체화하거나 기술적으로 결정해야 할 사항이 있으면 사용자에게 제시하고 논의한다.
**이 단계는 `claude-sonnet-4-6` 모델로 수행한다.**
